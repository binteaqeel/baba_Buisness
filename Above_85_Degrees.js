// Fully fixed version of your JS. Replace your existing Above_85_Degrees.js with this file.
...
function check() {
  try {
    ...
    // Per-mm values for each chamber
    ...
    const delivery_product_weight = delivered_gross_weight - delivered_tare_weight; // kg

    const received_gross_weight = num("Recieved_gross_weight");
    const received_tare_weight = num("Recieved_tare_weight");
    const received_product_weight = received_gross_weight - received_tare_weight; // kg

    // Averages (take only first N chambers as specified by No_Of_Chambers)
    ...
    // Product MT (using provided formula): kg * density / 1000
    ...
    // Title & header
    doc.setFontSize(14);
    doc.text("HSD 85 °F Table Calculation (Observed Temp > 85 °F)", 14, 15);
    ...
    const wbBody = [
      [
        "Dispatch",
        delivered_gross_weight.toFixed(2),
        delivered_tare_weight.toFixed(2),
        delivery_product_weight.toFixed(2),
        delivery_product_MT.toFixed(3),
      ],
      [
        "Receipt",
        received_gross_weight.toFixed(2),
        received_tare_weight.toFixed(2),
        received_product_weight.toFixed(2),
        received_product_MT.toFixed(3),
      ],
    ];
    ...
    const calcBody = [
      [
        "Dispatch",
        `${Dispatch_avg_temperature_C.toFixed(2)} / ${dispatched_in_Degree_F.toFixed(2)}`,
        dispatch_Temp_Difference.toFixed(2),
        delivery_avg_density.toFixed(3),
        HSD_Correction_Factor_Dispatch.toFixed(5),
        Factor_Dispatch.toFixed(6),
        Dispatched_Litre_at_85F.toFixed(2),
      ],
      [
        "Receipt",
        `${Received_avg_temperature_C.toFixed(2)} / ${received_in_Degree_F.toFixed(2)}`,
        received_Temp_Difference.toFixed(2),
        received_avg_density.toFixed(3),
        HSD_Correction_Factor_Received.toFixed(5),
        Factor_Received.toFixed(6),
        Received_Litre_at_85F.toFixed(2),
      ],
    ];
    ...
    console.error(err);
    // Also show a user-friendly message if not already alerted
